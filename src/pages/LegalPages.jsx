        ...
        {isSK ? (
          <>
            ...
            <ul>
              ...
              <li><strong>Chybové hlásenia:</strong> ak vám počas prihlásenej relácie zlyhá stránka, zaznamenáme text chyby, technický výpis, cestu k stránke (bez parametrov, ktoré ste zadali) a typ prehliadača. Slúži to výhradne na to, aby sme sa o poruche dozvedeli skôr než od vás; uchovávame ich 90 dní. Neprihláseným návštevníkom sa nezaznamenáva nič.</li>
              ...
              <li><strong>Fakturačné údaje:</strong> obchodné meno a fakturačná adresa objednávateľa, IČO a IČ DPH, ak ich uvediete pri objednávke. Zbierame ich preto, že bez nich nevieme vystaviť faktúru použiteľnú pre vaše účtovníctvo, a uchovávame ich podľa zákona o účtovníctve.</li>
              <li><strong>Správy, ktoré nám pošlete:</strong> obsah spätnej väzby alebo požiadavky na podporu vrátane snímky obrazovky, ak ju priložíte. Snímku prikladáte dobrovoľne — skontrolujte, čo je na nej vidno.</li>
            </ul>
            <p>
          ...
          <>
            ...
            <ul>
              ...
              <li><strong>Error reports:</strong> if a page fails while you are signed in, we record the error text, the technical trace, the page path (without any values you typed) and your browser type. It exists so we learn about a fault before you have to report it, and is kept for 90 days. Nothing is recorded for signed-out visitors.</li>
              ...
              <li><strong>Billing data:</strong> your business name and billing address, and your company registration and VAT numbers if you provide them at checkout. We collect these because without them we cannot issue an invoice your accounting can use, and we retain them under accounting law.</li>
              <li><strong>Messages you send us:</strong> the content of feedback or support requests, including a screenshot if you attach one. Attaching a screenshot is voluntary — check what it shows.</li>
            </ul>
            <p>
        ...
        {isSK ? (
          ...
          <>
            ...
            <ul>
              ...
              <li><strong>Webglobe, a. s.</strong> — mail servers for the residata.eu domain (its MX records) and our mailboxes; delivers and stores correspondence sent to @residata.eu addresses. Servers are in the European Union (Slovakia). <a href="https://www.webglobe.sk/gdpr" style={{ color: "var(--accent)" }} target="_blank" rel="noopener noreferrer">Webglobe Privacy Policy</a>.</li>
    ...
    <LegalPageShell title={isSK ? "Obchodné podmienky" : "Terms of Service"}>
      ...
      <Section title={isSK ? "3. Povaha dát — informatívna, nie poradenstvo" : "3. Nature of the data — informational, not advice"}>
        <p style={{ marginTop: 0 }}>{isSK
          ? "Dáta v Službe pochádzajú prevažne z verejných zdrojov (najmä webových stránok developerov) a sú spracované automatizovane. Vynakladáme primeranú snahu o ich presnosť a aktuálnosť, avšak NEZARUČUJEME úplnosť, presnosť, aktuálnosť ani vhodnosť dát na konkrétny účel. Zdroje sa môžu meniť, byť dočasne nedostupné alebo obsahovať chyby. Ako presne zbierame údaje, a ako môže developer požiadať o opravu alebo o zastavenie zberu, je uvedené na stránke Zdroje dát."
          : "The data in the Service comes largely from public sources (primarily developer websites) and is processed automatically. We make reasonable efforts toward accuracy and timeliness, but we DO NOT warrant the completeness, accuracy, currency, or fitness of the data for any particular purpose. Sources may change, be temporarily unavailable, or contain errors. How we collect it, and how a developer can request a correction or ask us to stop, is set out on our Data sources page."}</p>
        <p>{isSK
